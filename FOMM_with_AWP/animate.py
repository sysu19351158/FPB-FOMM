...
def animate(config, generator, kp_detector, checkpoint, log_dir, dataset):
    ...
    for it, x in tqdm(enumerate(dataloader)):
        with torch.no_grad():
            ...
            first_pridiction = generator(source_frame, kp_source=kp_source, kp_driving=kp_norm)['prediction']
            ...
            for frame_idx in range(driving_video.shape[2]):
                ...
                visualization = Visualizer(**config['visualizer_params']).visualize(source=source_frame,first_driving_frame=first_driving_frame,
                                                                                    first_pridiction=first_pridiction,
